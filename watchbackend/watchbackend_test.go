...
// fsHarness runs RunFsnotify on a background goroutine and buffers its fired ids so
...
func startFsnotify(t *testing.T, dirsByID map[string][]string) *fsHarness {
	...
	go func() {
		h.done <- RunFsnotify(ctx, dirsByID, func(id string) {
			select {
			case h.events <- id:
			default:
			}
	...
	t.Cleanup(func() {
		...
		case err := <-h.done:
			if !errors.Is(err, context.Canceled) {
				t.Errorf("RunFsnotify returned %v, want context.Canceled", err)
			}
		case <-time.After(2 * time.Second):
			t.Error("RunFsnotify did not return after cancel")
		}
	})
	return h
}
